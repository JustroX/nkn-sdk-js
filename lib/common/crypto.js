...
async function verify(publicKey, message, sign) {
  let result;
  ...
  try {
    ...
    result = _libsodiumWrappers.default.crypto_sign_verify_detached(Buffer.from(sign, "hex"), Buffer.from(message, "hex"), publicKey);
  } catch (e) {
    console.warn(e);
    result = _tweetnacl.default.sign.detached.verify(Buffer.from(sign, "hex"), Buffer.from(message, "hex"), publicKey);
  }
